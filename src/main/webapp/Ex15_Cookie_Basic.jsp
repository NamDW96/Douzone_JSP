...
2. local storage (key, value) 형태로 저장 >JSON 형태로 read, write
	장점) 크기 , 객체 사용 (데이터 : 의미있는 데이터 저장)
	
Server (웹 서버 (WAS) : 메모리 + 파일 , DB서버)
1. server memory : session 객체 (사이트 접속하는 사용자마다 고유하게 부여되는 메모리(객체) >서버 리부팅, 개발 코드 변경 >소멸(메모리 기반)> 생성 
2. server memory : application 객체 (사이트에 접속하는 모든 사용자에게 부여되는 메모리(객체) > >서버 리부팅, 개발 코드 변경 >소멸(메모리 기반)> 생성
...
<body>
	<%
		...
		//쿠기 새로고침하면 사라지지 않음, 삭제하고 새로고침하면 새로운 쿠키가 생성됨.
		//필요에 따라 쿠키를 읽거나 소멸 시킬 수 있다.
	%>
